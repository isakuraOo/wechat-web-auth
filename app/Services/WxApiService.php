...
class WxApiService
{
    ...
    const CONDITIONAL_MENU_CREATE_URL = '/menu/addconditional?';
    ...
    public $logcallback = './Runtime/Logs/wechat.log';
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function createConditionalMenu($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_URL_PREFIX.self::CONDITIONAL_MENU_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data));
        if ($result)
        {
            $json = json_decode($result,true);
            if (!$json || !empty($json['errcode'])) {
